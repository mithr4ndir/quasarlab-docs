...
- **Ansible owns the box.** All changes (`pg_hba.conf`, `postgresql.conf`, package versions) belong in `ansible-quasarlab`, in `roles/postgresql` and its `defaults/main.yml`, applied by `playbooks/postgresql.yml`. (This used to read `labctl-runs/postgres`; that directory was removed from the repo in `2ce0c19`.) Manual edits during incidents are allowed but must be back-ported the same day, and `pg_hba.conf` in particular is rendered wholesale from `postgresql_hba_entries`, so "the same day" is the actual deadline rather than a nicety.
